...
public class CpThemeGeneralActivity extends BaseActivity {
    @BindView(R.id.btTop3)
    ImageButton btTop3;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_cp_theme_general);

        ButterKnife.bind(this);

        btTop3.setImageResource(R.drawable.intro_button_03);

        /** 테마 대전 ListView, Adapter */
        competitionItemListAdapter = new CpThemeGeneralActivity.CompetitionItemListAdapter(this, R.layout.competition_list_item);
    ...
    private void getCpThemeList() {
        ...
        /** 통신처리 */
        client.init(R.string.cp_theme_list_by_me, pageno, 10, app_uid, "D");
        client.post(new BaseResponse<CompetitionThemeList>(CpThemeGeneralActivity.this) {
            @Override
            public void onResponse(CompetitionThemeList response) {
                LogHelper.debug(this, response.toString());
                if (!response.code.equals("000")) {
                    Toast.makeText(CpThemeGeneralActivity.this, response.msg, Toast.LENGTH_SHORT).show();
                    return;
                }
    ...
    public void btTop1Click() {
        Intent intent = new Intent(CpThemeGeneralActivity.this, CpThemeSpecialActivity.class);
    ...
    public void btTop2Click() {
        Intent intent = new Intent(CpThemeGeneralActivity.this, CpThemeEventActivity.class);
        startActivity(intent);
        finish();
    }

    @OnClick(R.id.btCreate)
    public void btCreateClick() {
        Toast.makeText(this, "테마만들기", Toast.LENGTH_SHORT).show();
/*
        Intent intent = new Intent(CpThemeGeneralActivity.this, CpThemeEventActivity.class);
        startActivity(intent);
//        finish();
        ...
        public void setListItem(CompetitionTheme item, CompetitionListItemHolder holder) {
            ...
            if (item.cnt_my_participate > 0) {
                ...
                holder.btStart.setOnClickListener(new View.OnClickListener() {
                    ...
                    public void onClick(View v) {
                        Toast.makeText(CpThemeGeneralActivity.this, "Done", Toast.LENGTH_SHORT).show();
                    }
                });
            }
            else {
                holder.btStart.setOnClickListener(new View.OnClickListener() {
                    @Override
                    public void onClick(View v) {
                        Toast.makeText(CpThemeGeneralActivity.this, "Start", Toast.LENGTH_SHORT).show();
                    }
                });
            }
            ...
            if (item.ct_status.equals("Y") && item.cnt_my_participate == 0) {
                ...
                holder.btResult.setOnClickListener(new View.OnClickListener() {
                    @Override
                    public void onClick(View v) {
                        Toast.makeText(CpThemeGeneralActivity.this, "Not yet", Toast.LENGTH_SHORT).show();
                    }
                });
            }
            else {
                holder.btResult.setOnClickListener(new View.OnClickListener() {
                    @Override
                    public void onClick(View v) {
                        Toast.makeText(CpThemeGeneralActivity.this, "Result", Toast.LENGTH_SHORT).show();
                    }
                });
            }

            Glide.with(CpThemeGeneralActivity.this).load(item.file_name_1).thumbnail(0.1f).into(holder.ivLeft);
            Glide.with(CpThemeGeneralActivity.this).load(item.file_name_2).thumbnail(0.1f).into(holder.ivRight);
        }
    }
}
